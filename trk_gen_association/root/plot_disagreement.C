...
void plot_disagreement()
{
    ...
    TH1F *hdr_signal = new TH1F("hdr_signal", "dr for matches that match to signal geometrically", x1bins, x1min, x1max);
    ...
    TH1F *hpt_signal = new TH1F("hpt_signal", "relpt for matches that match to signal geometrically", x2bins, x2min, x2max);
    ...
    for (Long64_t ient = 0; ient < nentries; ient++) {
        ...
        // Keep only tracks that do not have a match with conservative matching
        if (matchIdCut >= 0) continue;
        ...
        if (matchSta >= 100) {
            hdr_signal->Fill(matchDR);
            hpt_signal->Fill(matchRelPt);
        }
    } // end entries (track) loop 

    // Save histograms 
    TString rootf_out = "./histos/" + label + "_histos_disagreement.root";
    TFile *fout = new TFile(rootf_out, "recreate");
    std::cout << "(Re)creating " << rootf_out << " file." << std::endl;

    for (TH1F *h : {hdr, hpt,
                    hdr_bjets, hpt_bjets, 
                    hdr_signal, hpt_signal}) {
        h->Write();
    }
