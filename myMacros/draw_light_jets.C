void draw_light_jets(bool ktCut = false) 
{
    ...
    for (int i = 0; i < npt; i ++) {
        ...
        hpar_zg->GetYaxis()->SetRangeUser(0, ymax_zg + 0.5);
        hpar_rg->GetYaxis()->SetRangeUser(0, ymax_rg + 0.5);
        ...
        c->cd(i + 4);
        leg = new TLegend(0.35, 0.7, 0.85, 0.9);
        leg->SetBorderSize(0);
        leg->SetFillStyle(0);
        leg->AddEntry(hpar_zg, Form("#kern[-0.2]{ } parton"), "pl");
        leg->AddEntry(href_zg, Form("#kern[-0.2]{ } hadron"), "pl");
