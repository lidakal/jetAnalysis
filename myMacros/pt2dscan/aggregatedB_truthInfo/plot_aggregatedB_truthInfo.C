#include "TreeAnalyzer_aggregatedB_truthInfo.h"
#include "TH3.h"
#include <iostream>

using namespace std;

void plot_aggregatedB_truthInfo(bool parORreco = false, bool GSPincl = false)
{
    cout << "Running with options: " << endl;
    cout << "parORreco : " << parORreco << endl;
    cout << "GSPincl : " << GSPincl << endl;

    TreeAnalyzer_aggregatedB_truthInfo TAb;
    
    // Create output file name
    string foutname = "~/rootFiles/aggregatedB_truthInfo";
    if (!GSPincl) {
        foutname += "_noGSP";
    }
    ...
    for (Long64_t i = 0; i < TAb.nentries; i++) {
        ...
        for (Int_t j = 0; j < TAb.njet; j++) {
            ...
            // Fill dynKt histos with -1 when parORreco = true or when !jtIsHardest
            if (parORreco || (!TAb.jtIsHardest[j])) { 
                logrg = -1;
                logkt = -10;
            }
