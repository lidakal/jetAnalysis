...
void HistDrawer_pt2dscan::draw_rg_projection(bool GSPincl)
{
	...
	std::string savename = "rg_projections_1ptBin" + noGSP + ".png";
	c->Print(savename.c_str());
}

void HistDrawer_pt2dscan::draw_zg_projection(bool GSPincl)
{
    std::string noGSP = "";
    if (!GSPincl) {
        noGSP += "_noGSP";
    ...
    HistLoader_pt2dscan HL_ljet_ref(fname_chargedSJ_qcdMC_ref, "hL_zgkt");
    TH1D *h1d_ljet_ref = HL_ljet_ref.do_zg_projection(ptrange, "h1d_ljet_ref");
    h1d_ljet_ref->SetLineColor(1);
    h1d_ljet_ref->SetLineStyle(2);
    leg->AddEntry(h1d_ljet_ref, "l-jets, truth", "l");
    hs->Add(h1d_ljet_ref);

    HistLoader_pt2dscan HL_ljet_reco(fname_chargedSJ_qcdMC_reco, "hL_zgkt");
    TH1D *h1d_ljet_reco = HL_ljet_reco.do_zg_projection(ptrange, "h1d_ljet_reco");
    h1d_ljet_reco->SetLineColor(2);
    h1d_ljet_reco->SetLineStyle(2);
    leg->AddEntry(h1d_ljet_reco, "l-jets, reco", "l");
    hs->Add(h1d_ljet_reco);

    HistLoader_pt2dscan HL_bjet_ref(fname_merged_ref, "hBtag_zgkt");
    TH1D *h1d_bjet_ref = HL_bjet_ref.do_zg_projection(ptrange, "h1d_bjet_ref");
    h1d_bjet_ref->SetLineColor(4);
    h1d_bjet_ref->SetLineStyle(1);
    leg->AddEntry(h1d_bjet_ref, "b-jets, truth, aggregated pseudo-B's", "l");
    hs->Add(h1d_bjet_ref);

    HistLoader_pt2dscan HL_bjet_reco(fname_chargedSJ_bJetMC_reco, "hBtag_zgkt");
    TH1D *h1d_bjet_reco = HL_bjet_reco.do_zg_projection(ptrange, "h1d_bjet_reco");
    h1d_bjet_reco->SetLineColor(6);
    h1d_bjet_reco->SetLineStyle(1);
    leg->AddEntry(h1d_bjet_reco, "b-jets, reco, no aggregation", "l");
    hs->Add(h1d_bjet_reco);

    HistLoader_pt2dscan HL_bjet_reco_truth(fname_merged_truth_reco, "hBtag_zgkt");
    TH1D *h1d_bjet_reco_truth = HL_bjet_reco_truth.do_zg_projection(ptrange, "h1d_bjet_reco_truth");
    h1d_bjet_reco_truth->SetLineColor(7);
    h1d_bjet_reco_truth->SetLineStyle(1);
    leg->AddEntry(h1d_bjet_reco_truth, "b-jets, reco, aggregation with truth info", "l");
    hs->Add(h1d_bjet_reco_truth);

    HistLoader_pt2dscan HL_bjet_reco_SV(fname_merged_reco, "hBtag_zgkt");
    TH1D *h1d_bjet_reco_SV = HL_bjet_reco_SV.do_zg_projection(ptrange, "h1d_bjet_reco_SV");
    h1d_bjet_reco_SV->SetLineColor(8);
    h1d_bjet_reco_SV->SetLineStyle(1);
    leg->AddEntry(h1d_bjet_reco_SV, "b-jets, reco, aggregation if inSV", "l");
    hs->Add(h1d_bjet_reco_SV);

    HistLoader_pt2dscan HL_bjet_reco_highestEfficiency(fname_merged_highestEfficiency_reco, "hBtag_zgkt");
    TH1D *h1d_bjet_reco_highestEfficiency = HL_bjet_reco_highestEfficiency.do_zg_projection(ptrange, "h1d_bjet_reco_highestEfficiency");
    h1d_bjet_reco_highestEfficiency->SetLineColor(28);
    h1d_bjet_reco_highestEfficiency->SetLineStyle(1);
    leg->AddEntry(h1d_bjet_reco_highestEfficiency, "b-jets, reco, aggregation if (inSV || (|ip3dSig| > 3))", "l");
    hs->Add(h1d_bjet_reco_highestEfficiency);    
	...
	std::string ytitle = "1/N_{2-prong jets} dN/dz_{g}";
	hs->SetTitle(Form("; %s; %s", xtitle.c_str(), ytitle.c_str()));
	...
	std::string savename = "zg_projections_1ptBin" + noGSP + ".png";
	c->Print(savename.c_str());
}
