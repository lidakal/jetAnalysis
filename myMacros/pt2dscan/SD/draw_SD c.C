...
{      
    ...
    TH3D *h3d_ref = (TH3D *) fin_ref->Get("hC_rgkt")->Clone();
    TH3D *h3d_ref_dynKt = (TH3D *) fin_ref->Get("hC_rgkt_dynKt")->Clone();

    // Load par 3D histogram -- X = rg, Y = zg / kt, Z = pt 
    TFile *fin_par = new TFile(histfile_par.c_str());
    TH3D *h3d_par = (TH3D *) fin_par->Get("hC_rgkt")->Clone();
    ...
    string savename_c = "SD_cjets_par_vs_had";
