...
// Wed Mar  9 14:18:30 2022 by ROOT version 6.10/09
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           ngen;
   Int_t           genmatchindex[19];   //[ngen]
   Float_t         genpt[19];   //[ngen]
   Float_t         geneta[19];   //[ngen]
   Float_t         genphi[19];   //[ngen]
   Int_t           npar;
   Float_t         parpt[21];   //[npar]
   Float_t         pareta[21];   //[npar]
   Float_t         parphi[21];   //[npar]
   Int_t           parNb[21];   //[npar]
   Int_t           parNc[21];   //[npar]
   Bool_t          parHasGSPB[21];   //[npar]
   Bool_t          parHasGSPC[21];   //[npar]
   Bool_t          genIsHardest[19];   //[ngen]
   Float_t         gsjt1Pt[19];   //[ngen]
   Float_t         gsjt1Eta[19];   //[ngen]
   Float_t         gsjt1Phi[19];   //[ngen]
   Float_t         gsjt2Pt[19];   //[ngen]
   Float_t         gsjt2Eta[19];   //[ngen]
   Float_t         gsjt2Phi[19];   //[ngen]
   Float_t         psjt1Pt[19];   //[ngen]
   Float_t         psjt1Eta[19];   //[ngen]
   Float_t         psjt1Phi[19];   //[ngen]
   Float_t         psjt2Pt[19];   //[ngen]
   Float_t         psjt2Eta[19];   //[ngen]
   Float_t         psjt2Phi[19];   //[ngen]
   ...
   MyTree(const char *treename = "t", bool inclusive = true);
   virtual ~MyTree();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init_t(TTree *tree);
   virtual void     Init_HiTree(TTree *tree);
   virtual void     Loop();
...
#ifdef MyTree_cxx
MyTree::MyTree(const char *treename = "t", bool inclusive = true) : fChain(0) 
{
   TTree *tree;
   std::string filename;
   if (inclusive) {
      filename = "/data_CMS/cms/mnguyen//bJet2022/qcdMC/SD/merged_HiForestAOD.root";
   } else {
      filename = "/data_CMS/cms/mnguyen//bJet2022/bJetMC/SD/merged_HiForestAOD.root";
   }
   TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(filename.c_str());
   if (!f || !f->IsOpen()) {
      f = new TFile("/data_CMS/cms/mnguyen//bJet2022/qcdMC/SD/merged_HiForestAOD.root");
   }
   TDirectory * dir = (TDirectory*)f->Get("/data_CMS/cms/mnguyen//bJet2022/qcdMC/SD/merged_HiForestAOD.root:/ak4PFJetAnalyzer");
   dir->GetObject(treename,tree);

   }
   if (strcmp(treename, "t") == 0) { Init_t(tree); }
   if (strcmp(treename, "HiTree") == 0) { Init_HiTree(tree); }
}
...
void MyTree::Init_HiTree(TTree *tree)
{
   if (!tree) return;
   fChain = tree;
   fCurrent = -1;
