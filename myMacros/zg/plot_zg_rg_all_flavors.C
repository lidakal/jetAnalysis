...
void plot_zg_rg_all_flavors() 
{
    ...
    for (int ni = 0; ni < n; ni++) {
        ...
        */
        ...
        t->SetBranchStatus("*", 0);
        for (auto activeBranchName : {"ngen", "psjt1Pt", "psjt1Eta", "psjt1Phi", 
        ...
        // X = zg, Y = parpt
        TH2F *h_zg = new TH2F((hname + "_zg").c_str(), "parpt vs zg", 20, 0.01, 0.6, 20, 50, 100);
        
        // X = Rg, Y = parpt
        TH2F *h_rg = new TH2F((hname + "_rg").c_str(), "parpt vs rg", 20, 0.01, 1., 20, 50, 100);
        ...
        for (Long64_t i = 0; i < nentries; i++) {
            ...
            for (Long64_t j = 0; j < ngen; j++)  {
                Float_t zg = 0.;
                Float_t rg = 0.;
                Float_t minpt = min(psjt1Pt[j], psjt2Pt[j]);
                if (minpt > 0.) {
                    zg = minpt / (psjt2Pt[j]+psjt1Pt[j]);
                    Float_t DEta = psjt1Eta[j] - psjt2Eta[j];
                    Float_t DPhi = acos(cos(psjt1Phi[j] - psjt2Phi[j]));
                    rg = sqrt(DEta * DEta + DPhi * DPhi);
                }
                // Fill histograms
                h_zg->Fill(zg, parpt[j], weight);
                h_rg->Fill(rg, parpt[j], weight);
            }
        }
        hs_zg[ni] = h_zg;
        hs_rg[ni] = h_rg;
