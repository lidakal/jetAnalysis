...
public :
   ...
   Float_t         jer_sf_nom[500]; //[nref]
   Float_t         jer_sf_up[500]; //[nref]
   Float_t         jer_sf_down[500]; //[nref]
   Float_t         jec_unc[500]; //[nref]
...
//    Float_t jtParFlav[500]; //[nref]
   ...
   TBranch        *b_jer_sf_nom;   //!
   TBranch        *b_jer_sf_up;   //!
   TBranch        *b_jer_sf_down;   //!
   TBranch        *b_jec_unc;   //!
...
{
    std::cout << "Reading file: " << rootf << std::endl;
   TFile *fin = new TFile(rootf);
   if (rootf.Contains("LowEGJet") || rootf.Contains("HighEGJet") || rootf.Contains("SingleMuon")) {
    tree = (TTree*) fin->Get("akCs4PFJetAnalyzer/t"); // data tree
   } else {
    tree = (TTree*) fin->Get("ak4PFJetAnalyzer/t"); // MC tree
    tree->AddFriend("hiEvtAnalyzer/HiTree"); // to read the weight+pthat
   }
...
tTree::tTree(std::string rootf)
{
//    TFile *fin = new TFile(rootf);
    TFile *fin = TFile::Open(rootf.data());
    TString fname = rootf;
   if (fname.Contains("LowEGJet") || fname.Contains("HighEGJet") || fname.Contains("SingleMuon")) {
    tree = (TTree*) fin->Get("akCs4PFJetAnalyzer/t"); // data tree
   } else {
    tree = (TTree*) fin->Get("ak4PFJetAnalyzer/t"); // MC tree
...
void tTree::Init(TTree *tree)
{
   ...
   tree->SetBranchAddress("jer_sf_nom", jer_sf_nom, &b_jer_sf_nom);
   tree->SetBranchAddress("jer_sf_up", jer_sf_up, &b_jer_sf_up);
   tree->SetBranchAddress("jer_sf_down", jer_sf_down, &b_jer_sf_down);
   tree->SetBranchAddress("jec_unc", jec_unc, &b_jec_unc);
   ...
   tree->SetBranchAddress("HLT_HIL3Mu5_v1", &HLT_HIL3Mu5_v1, &b_HLT_HIL3Mu5_v1);
...
void tTree::plot_rgzgkt(TString foutname, Float_t bTagWP)
{
