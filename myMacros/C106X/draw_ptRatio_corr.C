...
void draw_ptRatio_corr()
{
    double ptMin = 100.;
    double ptMax = 120.;

    // ---- Grab histos ----
    TString indir = "./histos/qcd_dijet_";
    TString label = "aggrTMVA_withNb";
    TString fname = indir + label + "_ptRatio.root";
    TFile *fin = new TFile(fname);

    // gen for correction
    TH3D *hB_ptRatio_gen = (TH3D *) fin->Get("hB_ptRatio_gen");
    TH3D *hBtag_ptRatio_gen = (TH3D *) fin->Get("hBtag_ptRatio_gen");

    // reco tagged
    TH3D *hBtag_ptRatio = (TH3D *) fin->Get("hBtag_ptRatio");
    TH3D *hLtag_ptRatio = (TH3D *) fin->Get("hLtag_ptRatio");
    TH3D *hCtag_ptRatio = (TH3D *) fin->Get("hCtag_ptRatio");
    
    // Find pt bins
    Int_t izmin = hB_ptRatio_gen->GetZaxis()->FindBin(ptMin);
    Int_t izmax = hB_ptRatio_gen->GetZaxis()->FindBin(ptMax);

    // Find kt bins 
    // Int_t iymin = hB_ptRatio_gen->GetYaxis()->FindBin(0.);
    Int_t iymin = 0;
    Int_t iymax = hB_ptRatio_gen->GetNbinsY() + 1;

    // Make rg projections 
    THStack *hStack = new THStack ("hStack", "");
    hStack->SetTitle("; p_{T}^{B,ch} / p_{T}^{jet,ch}; 1/N dN/d(p_{T}^{B,ch} / p_{T}^{jet,ch})");

    TLegend *leg = new TLegend(0.2, 0.7, 0.5, 0.85);
    leg->SetHeader(Form("%.0f < p_{T}^{jet} < %.0f (GeV)", ptMin, ptMax));

    TH1F *hB_gen = (TH1F *) hB_ptRatio_gen->ProjectionX("hB_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_gen = (TH1F *) hBtag_ptRatio_gen->ProjectionX("hBtag_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_reco = (TH1F *) hBtag_ptRatio->ProjectionX("hBtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hLtag_reco = (TH1F *) hLtag_ptRatio->ProjectionX("hLtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hCtag_reco = (TH1F *) hCtag_ptRatio->ProjectionX("hCtag_reco", iymin, iymax, izmin, izmax);
    ...
    hStack_corrected->SetTitle("; p_{T}^{B,ch} / p_{T}^{jet,ch}; 1/N dN/d(p_{T}^{B,ch} / p_{T}^{jet,ch})");

    TLegend *leg_corrected = new TLegend(0.2, 0.7, 0.5, 0.85);
    leg_corrected->SetHeader(Form("%.0f < p_{T}^{jet} < %.0f (GeV)", ptMin, ptMax));
    ...
    hBtag_gen_ratio->GetXaxis()->SetTitle("m_{B}^{ch}");
    ...
    hStack_corrected->SetMaximum(4);
    leg_corrected->Draw();
    c_corrected->Draw();

    TCanvas *c_ratio = new TCanvas("c_ratio", "", 1200, 1000);
    hBtag_gen_ratio->Draw("histo");
    c_ratio->Draw();
}