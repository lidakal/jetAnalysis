...
void cl_frac_unc(TString observable="rg")
{
    ...
    TFile *fin_up = new TFile("../unfolding/histos/"+sample+"_"+label+"_unfolded_histograms_CLfracUp_"+observable+"_jer_nom_jec_nom_withSF.root");
    TH2D *h_up = (TH2D *) fin_up->Get("h_data_unfolded")->Clone("h_up");
    TFile *fin_down = new TFile("../unfolding/histos/"+sample+"_"+label+"_unfolded_histograms_CLfracDown_"+observable+"_jer_nom_jec_nom_withSF.root");
    ...
    TH1D *h_down_1d = (TH1D *) h_down->ProjectionX("h_down_1d", ibin_pt, ibin_pt);
    ...
    h_down_1d->SetMarkerColor(cmsViolet);
    h_down_1d->SetLineColor(cmsViolet);
    h_down_1d->SetMarkerSize(2);
    ...
    TH1D * h_unc_down = (TH1D *) h_down_1d->Clone("h_unc_down");
    h_unc_down->Add(h_nom_1d, -1);
    TH1D *h_unc_down_rel = (TH1D *) h_unc_down->Clone("h_unc_down_rel");
    ...
    for (int i=1; i<=h_unc_up_rel->GetNbinsX(); i++) {
        ...
        double unc2 = h_unc_down_rel->GetBinContent(i);
        h_unc_rel_sym_up->SetBinContent(i, std::max(std::abs(unc1),std::abs(unc2)));
    }
    ...
    if (observable=="zpt") leg = new TLegend(0.2, 0.2, 0.6, 0.5);
    leg->SetFillStyle(0);
    leg->SetBorderSize(0);
    leg->SetMargin(0.15);
    leg->AddEntry(h_nom_1d, "Nominal", "pe1");
    leg->AddEntry(h_up_1d, "Light+c fraction +100%", "pe1");
    leg->AddEntry(h_down_1d, "Light+c fraction -100%", "pe1");
    ...
    if (observable=="rg") h_nom_1d->GetYaxis()->SetRangeUser(0., 4.);
    else if (observable=="zg") h_nom_1d->GetYaxis()->SetRangeUser(0., 12.);
    ...
    if (observable=="rg") h_unc_up_rel->GetYaxis()->SetRangeUser(-0.2, 0.2);
    else if (observable=="zg") h_unc_up_rel->GetYaxis()->SetRangeUser(-0.2, 0.2);
    else if (observable=="zpt") h_unc_up_rel->GetYaxis()->SetRangeUser(-1., 1.);
    ...
    h_unc_down_rel->Draw("p hist same");

    c_unc->cd();
    top_pad->Draw();
    bottom_pad->Draw();
    c_unc->Draw();
    c_unc->Print("../plots_thesis/"+sample+"_"+label+"_cl_frac_unc_"+observable+".pdf");

    // Save histograms 
    TString fout_name = "histos/" + observable + "_cl_frac_XXT.root";
