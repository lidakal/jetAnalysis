...
void draw_unc_sym(TString observable="rg")
{
    ...
    h_model_fit_unc_rel->SetLineColor(cb_palette[5]);
    ...
    h_cl_frac_up_rel->SetLineColor(cb_palette[6]);
    ...
    h_sf_up_rel->SetLineColor(cb_palette[7]);
    ...
    TLegend *leg = new TLegend(0.2, 0.35, 0.5, 0.6); // abs scale
    ...
    leg->AddEntry(h_model_fit_unc_rel, "model templates", "pl");
    leg->AddEntry(h_trk_inef_up_rel, "3% trk inef", "pl");
    leg->AddEntry(h_mc_stat_unc_up_rel, "response stats", "pl");
    leg->AddEntry(h_sf_up_rel, "btag eff", "pl");
    leg->AddEntry(h_cl_frac_up_rel, "c+l fraction", "pl");
    ...
    if (observable!="zpt") {
        ...
        jet_info->SetNDC();
        jet_info->SetTextSize(20);
        jet_info->DrawLatex(0.2, 0.83, "anti-k_{T}, R=0.4 single b jets");
        ...
        jet_info->DrawLatex(0.45, 0.84, "anti-k_{T}, R=0.4 single b jets");
