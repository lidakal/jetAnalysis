...
void draw_unc_sym(TString observable="rg")
{
    ...
    TFile *fin_trk_inef = new TFile("histos/"+observable+"_trk_inef_unc_XXT.root");
    ...
    TFile *fin_btag = new TFile("histos/"+observable+"_sf_unc_XXT.root");
    TFile *fin_cl_frac = new TFile("histos/"+observable+"_cl_frac_XXT.root");
    ...
    TH1D *h_model_fit_unc_rel = (TH1D *) fin_model_fit_unc->Get("h_unc_rel_sym_up")->Clone("h_model_fit_unc");
    ...
    TH1D *h_cl_frac_up_rel = (TH1D *) fin_cl_frac->Get("h_unc_rel_sym_up")->Clone("h_cl_frac_unc");
    ...
    TH1D *h_sf_up_rel = (TH1D *) fin_btag->Get("h_unc_rel_sym_up")->Clone("h_sf_unc");
    ...
    if (observable!="zpt") {
        ...
        jet_info->DrawLatex(0.11, 0.47, "100 < p_{T}^{jet} < 120 GeV/c, |#eta^{jet}| < 2");
        jet_info->DrawLatex(0.11, 0.42, "Soft drop (charged particles)");
        jet_info->DrawLatex(0.11, 0.35, "z_{cut} = 0.1, #beta = 0, k_{T} > 1 GeV/c");
    } else {
        jet_info->SetTextAlign(30);
        jet_info->DrawLatex(0.93, 0.55, "anti-k_{T}, R = 0.4 b jets");
        jet_info->DrawLatex(0.93, 0.48, "100 < p_{T}^{jet} < 120 GeV/c, |#eta^{jet}| < 2");
    }
    ...
    c_unc->Print("../plots_thesis/total_unc_XXT_"+observable+".pdf");
}
