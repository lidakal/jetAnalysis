...
void stat_unc(TString observable="rg")
{
    ...
    for (int ibin_x = ibin_x_min; ibin_x <= ibin_x_max; ibin_x++) {
        double stat_unc = h_nom_1d->GetBinError(ibin_x);
        h_unc_up->SetBinContent(ibin_x, stat_unc);
    }
    TH1D *h_unc_down = (TH1D *) h_unc_up->Clone("h_unc_down");
    h_unc_down->Scale(-1.);
    ...
    TH1D *h_unc_rel_sym_up = (TH1D *) h_unc_up_rel->Clone("h_unc_rel_sym_up"); // already symmetric
    ...
    if (is_inclusive) h_unc_rel_sym_up->GetYaxis()->SetRangeUser(-0.2, 0.2);
    else if (observable=="rg") h_unc_rel_sym_up->GetYaxis()->SetRangeUser(-0.4, 0.4);
    else if (observable=="zg") h_unc_rel_sym_up->GetYaxis()->SetRangeUser(-0.2, 0.2);
    else if (observable=="zpt") h_unc_rel_sym_up->GetYaxis()->SetRangeUser(-0.4, 0.4);
