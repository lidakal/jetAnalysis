...
void trk_inef_unc(TString observable="rg")
{
    ...
    TString sample1 = "pythia_PF40";
    TString sample2 = "pythia_0_03_trk_inef_PF40";
    TString label = "aggrTMVA_XXT";
    bool is_inclusive = label.Contains("inclusive");

    TString suffix = (is_inclusive) ? "" : "_withSF";

    TFile *fin_nom = new TFile("../unfolding/histos/"+sample1+"_"+label+"_unfolded_histograms_"+observable+"_jer_nom_jec_nom" + suffix + ".root");
    TH2D *h_nom = (TH2D *) fin_nom->Get("h_data_unfolded")->Clone("h_nom");
    TFile *fin_up = new TFile("../unfolding/histos/"+sample2+"_"+label+"_unfolded_histograms_"+observable+"_jer_nom_jec_nom" + suffix + ".root");
    ...
    leg->AddEntry(h_up_1d, "3% tracking inefficiency", "pe1");
    ...
    if (is_inclusive) h_unc_up_rel->GetYaxis()->SetRangeUser(-0.2, 0.2);
    ...
    bottom_pad->Draw();
    c_unc->Draw();
    c_unc->Print("../plots_thesis/"+sample1+"_"+label+"_trk_inef_unc_"+observable+".pdf");

    // Save histograms 
    TString suffix_out = (is_inclusive) ? "inclusive" : "XXT";
    TString fout_name = "histos/" + observable + "_trk_inef_unc_" + suffix_out + ".root";
