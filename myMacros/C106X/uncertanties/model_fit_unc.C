...
void model_fit_unc(TString observable="rg")
{
    ...
    TFile *fout = new TFile("./histos/"+observable+"_model_fit_unc_XXT.root", "recreate");
    ...
    for (int ibin_pt = 1; ibin_pt <= nbins_pt; ibin_pt++) {
        ...
        TH1D * h_model_fit_unc = (TH1D *) h_her_1d->Clone(Form("h_model_fit_unc_%d", ibin_pt));
        h_model_fit_unc->Add(h_nom_1d, -1);
        h_model_fit_unc->GetYaxis()->SetTitle("var-nom");
        h_model_fit_unc->GetYaxis()->SetTitleOffset(2.);
        h_model_fit_unc->GetXaxis()->SetTitle(xlabel);
        h_model_fit_unc->GetXaxis()->SetTitleOffset(3.);
        ...
        // fout->cd();
        h_model_fit_unc->Write();
        h_model_fit_unc_rel->Write();

    }
    c_model->Draw();
    c_model->Print("plots_an/model_fit_unc_XXT_"+observable+".png");
    // fout->Close();
    // delete fout;
}