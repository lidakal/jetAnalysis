...
void bottomline_test_JECsrc(TString observable="rg", TString jer_opt="nom", TString jec_opt="up_JECsrc")
{
    // ---------------- Options -------------------
    bool withSF = true;
    bool sfDown = false;
    ...
    TString fname_response_unfolding = "./histos/"+sample_unfolding+"_"+label_unfolding+"_response" + suffix_in + ".root";
    ...
    TString fname_b_tag_eff = "../btag/histos/"+sample_sf+"_"+label_sf+"_" +observable + "_efficiency_jer_nom_jec_nom_withSF.root";
    ...
    for (int i=0; i<28; i++) {
        // if (i>0) break;
        TH2D *h_mc_reco = (TH2D *) fin_response_unfolding->Get(Form("h%d_purity_numerator_%spt", i, observable.Data())); // reco MC to compare w/ data after purity correction
        TH2D *h_purity = (TH2D *) fin_response_unfolding->Get(Form("h%d_purity_%spt", i, observable.Data())); // reconstruction purity correction
        TH2D *h_efficiency = (TH2D *) fin_response_unfolding->Get(Form("h%d_efficiency_%spt", i, observable.Data())); // reconsturction efficiency correction
        RooUnfoldResponse *response = (RooUnfoldResponse *) fin_response_unfolding->Get(Form("response%d_%spt", i, observable.Data())); // response 
        TH2D *h_mc_btag_true = (TH2D *) fin_response_unfolding->Get(Form("h%d_efficiency_numerator_%spt", i, observable.Data())); // true MC to compare w/ data BEFORE efficiency corrections
        ...
        TH2D *h_mc_true = (TH2D *) fin_response_truth->Get(Form("h%d_efficiency_denominator_%spt", i, observable.Data())); // true MC to compare w/ data AFTER efficiency corrections

        //------- Apply purity correction
        std::cout << "\t---->Multiplying data by purity" << std::endl;
        TH2D *h_data_purity_corrected = (TH2D *) h_data_after_fit->Clone(Form("h%d_data_purity_corrected",i));
        ...
        h_data_unfolded->SetName(Form("h%d_data_unfolded",i));

        // ---- Fold back
        std::cout << "\t---->Refolding" << std::endl;
        TH2D *h_data_refolded = (TH2D *) response->ApplyToTruth(h_data_unfolded, Form("h%d_data_refolded",i));

        // ---- Apply efficiency correction 
        std::cout << "\t---->Dividing by recostruction efficiency" << std::endl;
        TH2D *h_data_efficiency_corrected = (TH2D *) h_data_unfolded->Clone(Form("h%d_data_efficiency_corrected",i));
        h_data_efficiency_corrected->Divide(h_efficiency);

        // ---- Grab b tagging efficiency correction 
        TH2D *h_data_fully_corrected = (TH2D *) h_data_efficiency_corrected->Clone(Form("h%d_data_fully_corrected",i));
        std::cout << "\t---->Dividing by b tagging efficiency (with SF)" << std::endl;
        TH2D *h_eff_withSF = (TH2D *) fin_b_tag_eff->Get(Form("h%d_eff_withSF",i));
        ...
        if (false) {
            ...
            TH1D *h_mc_reco_1d = (TH1D *) h_mc_reco->ProjectionX(Form("h%d_mc_reco_1d",i), ibin_pt, ibin_pt);
            TH1D *h_mc_true_1d = (TH1D *) h_mc_true->ProjectionX(Form("h%d_mc_true_1d",i), ibin_pt, ibin_pt);
            TH1D *h_data_purity_corrected_1d = (TH1D *) h_data_purity_corrected->ProjectionX(Form("h%d_data_purity_corrected_1d",i), ibin_pt, ibin_pt);
            TH1D *h_data_fully_corrected_1d = (TH1D *) h_data_fully_corrected->ProjectionX(Form("h%d_data_fully_corrected_1d",i), ibin_pt, ibin_pt);
            TH1D *h_data_refolded_1d = (TH1D *) h_data_refolded->ProjectionX(Form("h%d_data_refolded_1d",i), ibin_pt, ibin_pt);
            ...
            TCanvas *c_unfold = new TCanvas(Form("c%d_unfold",i), "", 800, 600);
    ...
    for (int i=0; i<28; i++) {
        unfolded_histos[i]->SetName(Form("h%d_data_unfolded",i));
        unfolded_histos[i]->Write();
    }
