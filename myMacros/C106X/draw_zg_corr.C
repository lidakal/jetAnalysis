...
void draw_zg_corr()
{
    ...
    TString fname = indir + label + "_zgkt.root";
    TFile *fin = new TFile(fname);

    // gen for correction
    TH3D *hB_zgkt_gen = (TH3D *) fin->Get("hB_zgkt_gen");
    TH3D *hBtag_zgkt_gen = (TH3D *) fin->Get("hBtag_zgkt_gen");

    // reco tagged
    TH3D *hBtag_zgkt = (TH3D *) fin->Get("hBtag_zgkt");
    TH3D *hLtag_zgkt = (TH3D *) fin->Get("hLtag_zgkt");
    TH3D *hCtag_zgkt = (TH3D *) fin->Get("hCtag_zgkt");
    
    // Find pt bins
    Int_t izmin = hB_zgkt_gen->GetZaxis()->FindBin(ptMin);
    Int_t izmax = hB_zgkt_gen->GetZaxis()->FindBin(ptMax);

    // Find kt bins 
    double ktMin = 1.;
    Int_t iymin = hB_zgkt_gen->GetYaxis()->FindBin(std::log(ktMin));
    Int_t iymax = hB_zgkt_gen->GetNbinsY();

    // Make rg projections 
    THStack *hStack = new THStack ("hStack", "");
    hStack->SetTitle("; z_{g} = p_{T,2} / (p_{T,1} + p_{T,2}); 1/N dN/d(z_{g})");

    TLegend *leg = new TLegend(0.45, 0.6, 0.8, 0.85);
    leg->SetHeader(Form("%.0f < p_{T}^{jet} < %.0f (GeV), k_{T} > %.0f GeV", ptMin, ptMax, ktMin));

    TH1F *hB_gen = (TH1F *) hB_zgkt_gen->ProjectionX("hB_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_gen = (TH1F *) hBtag_zgkt_gen->ProjectionX("hBtag_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_reco = (TH1F *) hBtag_zgkt->ProjectionX("hBtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hLtag_reco = (TH1F *) hLtag_zgkt->ProjectionX("hLtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hCtag_reco = (TH1F *) hCtag_zgkt->ProjectionX("hCtag_reco", iymin, iymax, izmin, izmax);
    ...
    hStack_corrected->SetTitle("; z_{g} = p_{T,2} / (p_{T,1} + p_{T,2}); 1/N dN/d(z_{g})");

    TLegend *leg_corrected = new TLegend(0.45, 0.6, 0.8, 0.85);
    ...
    hBtag_gen_ratio->GetXaxis()->SetTitle("z_{g} = p_{T,2} / (p_{T,1} + p_{T,2})");
