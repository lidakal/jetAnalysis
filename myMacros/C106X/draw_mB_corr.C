...
void draw_mB_corr()
{
    ...
    TString fname = indir + label + "_mB.root";
    TFile *fin = new TFile(fname);

    // gen for correction
    TH3D *hB_mB_gen = (TH3D *) fin->Get("hB_mB_gen");
    TH3D *hBtag_mB_gen = (TH3D *) fin->Get("hBtag_mB_gen");

    // reco tagged
    TH3D *hBtag_mB = (TH3D *) fin->Get("hBtag_mB");
    TH3D *hLtag_mB = (TH3D *) fin->Get("hLtag_mB");
    TH3D *hCtag_mB = (TH3D *) fin->Get("hCtag_mB");
    
    // Find pt bins
    Int_t izmin = hB_mB_gen->GetZaxis()->FindBin(ptMin);
    Int_t izmax = hB_mB_gen->GetZaxis()->FindBin(ptMax);

    // Find kt bins 
    // Int_t iymin = hB_mB_gen->GetYaxis()->FindBin(0.);
    Int_t iymin = 0;
    Int_t iymax = hB_mB_gen->GetNbinsY() + 1;

    // Make rg projections 
    THStack *hStack = new THStack ("hStack", "");
    hStack->SetTitle("; m_{B}^{ch}; 1/N dN/d(m_{B}^{ch})");

    TLegend *leg = new TLegend(0.5, 0.7, 0.8, 0.85);
    leg->SetHeader(Form("%.0f < p_{T}^{jet} < %.0f (GeV)", ptMin, ptMax));

    TH1F *hB_gen = (TH1F *) hB_mB_gen->ProjectionX("hB_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_gen = (TH1F *) hBtag_mB_gen->ProjectionX("hBtag_gen", iymin, iymax, izmin, izmax);
    ...
    TH1F *hBtag_reco = (TH1F *) hBtag_mB->ProjectionX("hBtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hLtag_reco = (TH1F *) hLtag_mB->ProjectionX("hLtag_reco", iymin, iymax, izmin, izmax);
    TH1F *hCtag_reco = (TH1F *) hCtag_mB->ProjectionX("hCtag_reco", iymin, iymax, izmin, izmax);
    ...
    hStack_corrected->SetTitle("; m_{B}^{ch}; 1/N dN/d(m_{B}^{ch})");
    ...
    hStack_corrected->SetMaximum(0.6);
    leg_corrected->Draw();
    c_corrected->Draw();
