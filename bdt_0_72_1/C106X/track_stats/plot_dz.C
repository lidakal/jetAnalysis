...
void plot_dz()
{
    ...
    TString odir = "./histos/";
    TString fout = odir + label + "_" + ptMin + "_pt_" + ptMax + "_dz.root";    
    ...
    TH2F *h_dz = new TH2F("h_dz", "x=dz, y=class", x1bins, x1min, x1max, y1bins, y1min, y1max);
    TH2F *h_dz_bjet = new TH2F("h_dz_bjet", "x=dz, y=class", x1bins, x1min, x1max, y1bins, y1min, y1max);
    ...
    for (Long64_t ient = 0; ient < t.GetEntries(); ient++) {
        ...
        for (Int_t itrk = 0; itrk < t.ntrk; itrk++) {
            ...
            // Fill histos
            h_dz->Fill(trkDz, yclass);
            if (isBjet && passBtag) {
                h_dz_bjet->Fill(trkDz, yclass);
            }
        } // end track loop
    } // end entry loop
